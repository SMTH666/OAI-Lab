...
plt.title('Custom Clustering (n_clusters: 3, rseed: 2)')
plt.show()

centers, labels = find_clusters(X, 3, rseed=0)
print("n_clusters: 3, rseed: 0")
plt.scatter(X[:, 0], X[:, 1], c=labels, s=50, cmap='viridis')
plt.title('Custom Clustering (n_clusters: 3, rseed: 0)')
plt.show()

# Використання KMeans з вказаною кількістю кластерів та random_state
labels = KMeans(3, random_state=0).fit_predict(X)
print("using KMeans():")
print("n_clusters: 3, random_state: 0")
plt.scatter(X[:, 0], X[:, 1], c=labels, s=50, cmap='viridis')
plt.title('KMeans Clustering (n_clusters: 3, random_state: 0)')
plt.show()
